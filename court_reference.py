...
class CourtReference:
    ...
    def __init__(self):
        ...
        # Try to load the court reference image, if it doesn't exist, build it
        import os
        court_path = 'court_configurations/court_reference.png'
        if os.path.exists(court_path):
            self.court = cv2.cvtColor(cv2.imread(court_path), cv2.COLOR_BGR2GRAY)
        else:
            # Create the directory if it doesn't exist
            os.makedirs('court_configurations', exist_ok=True)
            # Build the court reference image
