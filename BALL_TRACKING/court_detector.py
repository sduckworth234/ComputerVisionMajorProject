...
MODEL_PKL = "/Users/owendyne/Documents/GitHub/ComputerVisionMajorProject/BALL_TRACKING/court_validity_knn(92946_images).pkl"
IMAGE_PATH = "/Users/owendyne/Documents/GitHub/ComputerVisionMajorProject/data/DATA/LABLED_FRAMES/ao_w_2024/INVALID/frame_000238.png"

# ------------------------------
# Model I/O (pickle) - self contained
# ------------------------------
...
# ------------------------------
# KNN prediction (Euclidean) with uniform/distance weights
# ------------------------------
def _knn_predict_batch(Xtr: np.ndarray, ytr: np.ndarray, X: np.ndarray, k: int = 7, weights: str = "distance") -> np.ndarray:
    """
    Vectorised KNN for small datasets.
    Xtr: [Ntr, D], ytr: [Ntr], X: [N, D]
    Returns yhat: [N]
    """
...
# ------------------------------
# Feature extraction (BGR -> dict of floats)
# Note: If the saved model expects a specific feature set, the keys in `feature_names`
# must exist in the returned dict. Unknown keys are filled with 0.0 and a single warning is printed.
# ------------------------------
...
def _white_line_mask(bgr: np.ndarray) -> np.ndarray:
    """Colour-invariant(ish) white mask: bright & low-sat in HSV/HLS, then morph close."""
...
def _vectorise_features_from_names(feats: Dict[str, float], feature_names: List[str]) -> np.ndarray:
    """
    Convert dict -> row vector in the order of `feature_names`.
    Unknown feature names are filled with 0.0 (warn once).
    """
...
def predict_image_file_with_pickle(image_path: str, pkl_path: str = MODEL_PKL) -> bool:
    """Load an image from disk and return True if VALID, else False, using the saved pickle model."""
